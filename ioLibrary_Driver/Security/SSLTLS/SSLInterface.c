...
int wiz_tls_init(wiz_tls_context* tlsContext, int* socket_fd, const char * host)
{
	...
	*/
    ...
    if (ssl_option->client_cert_enable == ENABLE)
    {
        ...
        //memcpy(g_pkey_buf, private_key, strlen(private_key));

#if USE_SECURE_FLASH
        SECURE_FLASH_READ(STORAGE_PKEY, (uint8_t *)g_pkey_buf, ssl_option->pkey_len);

#else
        //read_storage(STORAGE_ROOTCA, ,(uint8_t *)g_rootca_buf, dev_config->ssl_option.rootca_len + 1);
#endif

        //ret = mbedtls_x509_crt_parse((tlsContext->clicert), (const char *)client_cert, strlen(client_cert) + 1);
        ...
        //ret = mbedtls_pk_parse_key(tlsContext->pkey, (const char *)private_key, strlen(private_key) + 1, NULL, 0);
...
#if 0
    if((ret = mbedtls_ssl_conf_alpn_protocols(tlsContext->conf, alpnProtocols)) != 0) {
